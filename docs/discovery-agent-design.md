...
**Coverage delta — not currently available.** `coverage` is populated per file
as executed line lists, but *only for concrete cases*: every case generated by
the CrossHair backend records `coverage: null`, because Hypothesis's line
tracer cannot run alongside CrossHair's. Coverage reported from a solver run
therefore describes its concrete phases, and reading it as solver reach is a
mistake this project made twice before checking.

Were it available, comparing the baseline's union against the solver's would be
the best progress signal on offer: continuous, per-test, needing no planted
fault, and immune to the small-domain realization problem. It would still not
subsume a canary — line coverage is insensitive to *which* values ran through a
line, so a narrow value-dependent bug is reached, and missed, at identical
coverage. See backlog B14.
