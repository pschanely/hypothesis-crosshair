...
class SpanTracker:
    ...
    def span_start(self, label: int) -> None:
        ...
        debug("span_start label=", label, " new depth=", self.depth())

    def span_end(self) -> None:
        if self.spans:
            label = self.spans.pop()
            self.span_depths[label] -= 1
            debug("span_end, discard=", False, " new depth=", self.depth())
        else:
...
class CrossHairPrimitiveProvider(PrimitiveProvider):
    ...
    def per_test_case_context_manager(self):
        ...
        if self.span_tracker.spans:
            ...
            debug("span mismatch; labels started but not ended:", self.span_tracker.spans)
