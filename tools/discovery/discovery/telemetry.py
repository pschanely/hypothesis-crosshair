...
#: Hypothesis records ``coverage: null`` for cases generated by the CrossHair
#: backend - its own line tracer cannot run alongside CrossHair's. Coverage in
#: a solver run therefore describes the concrete phases only, and says nothing
#: about what the solver reached.
COVERAGE_UNAVAILABLE_UNDER_CROSSHAIR = True
